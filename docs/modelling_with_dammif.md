...
RAW offers a nice GUI for running *ab initio* dummy atom modelling from SAXS data. If you rather run DAMMIF *et al.* from the command prompt, go to the [next section](/modelling_with_dammif/#dammifdammin-via-the-command-prompt). Otherwise, in RAW, right-click on the GNOM `.out` file and select `Bead Model (DAMMF/N)`. This will open a new dialog window.
...
| "Type" of modelling       | Quick and dirty |  Good | Ultimate |
|---------------------------|-----------------|-------|----------|
...
RAW also outputs a CSV file (`lys_8_dammif_results.csv` in our case) containing the data presented in the table above. Additionnaly, a PDF file (`lys_8_dammif_results.pdf` here) is printed where the resulting SAXS profile of each model is fitted to the experimental data.
